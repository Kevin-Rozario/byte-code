...
const mailGenerator = new Mailgen({
  ...
  product: {
    name: "ByteCode",
    link: process.env.APP_URL || "https://byteCode.example.com",
...
export const emailVerificationMailGenContent = ({
  ...
  return {
    body: {
      name: userName,
      intro: "Welcome to ByteCode! We're very excited to have you on board.",
      action: {
        instructions:
          "To get started with ByteCode and verify your email address, please click the button below:",
        button: {
          color: "#007bff",
...
export const passwordResetMailGenContent = ({ userName, passwordResetUrl }) => {
  return {
    body: {
      name: userName,
      intro:
        "You are receiving this email because a password reset request has been initiated for your ByteCode account.",
      action: {
